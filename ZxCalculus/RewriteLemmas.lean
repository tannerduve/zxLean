...
lemma color_change_Z_1 (α : ZMod 8) :
...
lemma color_change_X_1 (α : ZMod 8) :
...
theorem pauliX_as_HZH : ZxEquiv PauliX (H ; PauliZ ; H) := by
  have h := by simpa [Nat.reduceMul, tensor_pow_H_one, eq_mpr_eq_cast, cast_eq] using @color_change_Z 4 1 1
  exact symm h

/-- Pauli Z gate can be expressed as H ; PauliX ; H -/
theorem pauliZ_as_HXH : ZxEquiv PauliZ (H ; PauliX ; H) := by
  have h := by simpa [Nat.reduceMul, tensor_pow_H_one, eq_mpr_eq_cast, cast_eq] using @color_change_X 4 1 1
  exact symm h
...
theorem z_cancel (α : ZMod 8) : ZxEquiv (Z α 1 1 ; Z (-α) 1 1) id := by
...
theorem x_cancel (α : ZMod 8) : ZxEquiv (X α 1 1 ; X (-α) 1 1) id := by
